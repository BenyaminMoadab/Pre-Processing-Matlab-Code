...
if strcmpi(button, 'Floating Point')
	...
	message = sprintf('uint8 or uint16?');
	button = questdlg(message, 'Number of Bits?', 'uint8', 'uint16', 'Cancel', 'uint8');
	if contains(button, '16')
		% Convert the 8 bit image to uint16.
		grayImage = double(grayImage);
		% Add noise so that we'll have a continuum of gray levels represented.
		noiseImage = rand(rows, columns) - 0.5;
		grayImage = uint16( 200 * (grayImage + noiseImage));
		startingLowThreshold = 7;
		startingHighThreshold = 23*256;
		numberOfHistogramBins = 512;
	else
		% uint8
		startingLowThreshold = 7;
	...
	[pixelCount, grayLevels] = imhist(grayImage, numberOfHistogramBins);
	subplot(2, 3, 2); 
	bar(grayLevels, pixelCount, 'BarWidth', 1, 'FaceColor', 'b');
